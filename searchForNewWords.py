...
def replace_with_colored(match_obj, color):
    ...
    print(match_obj.group(2), end=" ")
    return match_obj.group(1) + color + match_obj.group(2) + colorama.Fore.RESET + match_obj.group(3)

def searchForNewWords(dictionary_path, word_search_path, show_sentences):
    ...
    if show_sentences:
        for i in range(len(word_search_lines)):
            for word in known_words:
                word_search_lines[i] = re.sub("([\s,.;^\"])({})([\s,.;!?$\"])".format(word), lambda match_obj : replace_with_colored(match_obj, colorama.Fore.GREEN), word_search_lines[i], flags=re.IGNORECASE)
            for word in unknown_words:
                word_search_lines[i] = re.sub("([\s,.;^\"])({})([\s,.;!?$\"])".format(word), lambda match_obj : replace_with_colored(match_obj, colorama.Fore.RED), word_search_lines[i], flags=re.IGNORECASE)
        print("\nSentences:\n")
